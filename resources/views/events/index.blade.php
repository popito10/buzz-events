...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="text-center mb-8 px-4">
        ...
        <p class="text-gray-500 text-sm sm:text-base">Découvrez les dernières tendances et actualités du web</p>
    </div>
    ...
    <div class="mb-8 max-w-xl mx-auto">
        <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
            ...
            <button onclick="searchEvents()" class="bg-purple-600 text-white px-6 py-3 rounded-lg hover:bg-purple-700 transition font-semibold">
    ...
    @if($events->count() > 0)
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6" id="eventsGrid">
                ...
                <div class="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden flex flex-col">
                    ...
                    <div class="flex flex-wrap gap-2 text-xs text-gray-400 mb-4">
                        ...
                        <div class="flex space-x-2">
                            <a href="{{ route('events.show', $event) }}" class="flex-1 bg-purple-600 text-white text-sm px-3 py-2 rounded-lg hover:bg-purple-700 transition text-center flex items-center justify-center">
                                <i class="fas fa-eye mr-1"></i> Voir
                            </a>
                            <a href="{{ $event->source_link }}" target="_blank" class="flex-1 bg-pink-600 text-white text-sm px-3 py-2 rounded-lg hover:bg-pink-700 transition text-center flex items-center justify-center">
    ...
    @else
        ...
        <div class="text-center py-16 px-4">
            ...
            <p class="text-gray-500 text-lg mb-2">Aucun événement pour le moment</p>
            <p class="text-gray-400 text-sm mb-6">Soyez le premier à partager un événement !</p>
            <a href="{{ route('events.create') }}" class="inline-block bg-purple-600 text-white px-6 py-3 rounded-lg hover:bg-purple-700 transition font-semibold">
...
<script>
    function searchEvents() {
        ...
        cards.forEach(card => {
            ...
            const description = card.querySelector('.text-gray-500').textContent.toLowerCase();
